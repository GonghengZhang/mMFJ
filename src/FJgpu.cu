...
__global__ void sum_J(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
...
__global__ void trap_J1(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
	...
	if (validr) {
		...
		for (int i = 1; i < nr; i++) {
			...
			r2 = r[i];
			dr0 = fmaxf((r2 - r1), 0.1);
			kernel += (g1*j1(k*r1)*r1+g2*j1(k*r2)*r2)*dr0/2;
		}
		out[indx] = kernel;
	}
}

__global__ void trap_J2(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
	...
	if (validr) {
		...
		for (int i = 1; i < nr; i++) {
			...
			r2 = r[i];
			dr0 = fmaxf((r2 - r1), 0.1);
			kernel += (g1*jn(2,k*r1)*r1+g2*jn(2,k*r2)*r2)*dr0/2;
		}
		out[indx] = kernel;
	}
}

__global__ void sum_Y(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
...
__global__ void trap_Y1(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
	...
	if (validr) {
		...
		for (int i = 1; i < nr; i++) {
			...
			r2 = r[i];
			dr0 = fmaxf((r2 - r1), 0.1);
			kernel += (g1*y1(k*r1)*r1+g2*(y1(k*r2))*r2)*dr0/2;
		}
		out[indx] = kernel;
	}
}

__global__ void trap_Y2(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
	...
	if (validr) {
		...
		for (int i = 1; i < nr; i++) {
			...
			r2 = r[i];
			dr0 = fmaxf((r2 - r1), 0.1);
			kernel += (g1*yn(2,k*r1)*r1+g2*yn(2,k*r2)*r2)*dr0/2;
		}
		out[indx] = kernel;
	}
}

__global__ void trap_Y0_Y2(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
	...
	if (validr) {
		...
		for (int i = 1; i < nr; i++) 
		{
			...
			r1 = r[i - 1];
			r2 = r[i];
			dr0 = fmaxf((r2 - r1), 0.001);
			kernel += (g1*(y0(k*r1) - yn(2,k*r1))*r1+g2*(y0(k*r2)-yn(2,k*r2))*r2)*dr0/2;
		}
		out[indx] = kernel;
	}
}

__global__ void trap_Y0Y2(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
...
__global__ void trap_J0J2(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
	...
	if (validr) {
		...
		for (int i = 1; i < nr; i++) 
		{
			...
			r2 = r[i];
			dr0 = fmaxf((r2 - r1), 0.001);
			kernel += (g1*(j0(k*r1) + jn(2,k*r1))*r1+g2*(j0(k*r2) + jn(2,k*r2))*r2)*dr0/2;
		}
		out[indx] = kernel;
	}
}

__global__ void trap_J0_J2(float *U_f, float *r, float *out, float *c, float *f, int nr, int nc, int nf) {
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
...
__global__ void trap_cj2(float *U_f,float *r,float *out,float *c,float *f,int nr,int nc,int nf){
	bool validr = true;
	const int gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int gtidy = blockIdx.y * blockDim.y + threadIdx.y;
	int indx = 0;
	...
	if(validr){
		...
		for(int i=1;i<nr;i++){
			...
			r1 = r[i-1];
			r2 = r[i];
			dr0 = fmaxf((r2 - r1),0.0001);
			kernel += 0.5*(jnf(2,k*r1)*g1*r1+jnf(2,k*r2)*g2*r2)*dr0;
		}
		out[indx] = kernel;
	}
}
...
{	
	...
	if(type==2)
		trap_cj2<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
	if(type==1)
...
int FHr(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
    ...
    checkCudaErrors(cudaMemcpy(cc,c,nc*sizeof(float),cudaMemcpyHostToDevice));
    if(type==2)
	sum_J<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    if(type==1)
...
int FHi(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
	...
	checkCudaErrors(cudaMemcpy(cc,c,nc*sizeof(float),cudaMemcpyHostToDevice));
	if(type==2)
		sum_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
	if(type==1)
...
int FY0_Y2(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
	...
	//	integral_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
	if(type==0)
		trap_Y0_Y2<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    checkCudaErrors(cudaMemcpy(out, outc, nf*nc*sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(u_fc));
    checkCudaErrors(cudaFree(rc));
    checkCudaErrors(cudaFree(fc));
...
int FY0Y2(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
	...
	//	integral_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
	if(type==0)
		trap_Y0Y2<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    checkCudaErrors(cudaMemcpy(out, outc, nf*nc*sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(u_fc));
    checkCudaErrors(cudaFree(rc));
    checkCudaErrors(cudaFree(fc));
...
int FJ0J2(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
	...
	//	integral_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
	if(type==0)
		trap_J0J2<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    checkCudaErrors(cudaMemcpy(out, outc, nf*nc*sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(u_fc));
    checkCudaErrors(cudaFree(rc));
    checkCudaErrors(cudaFree(fc));
...
int FJ0_J2(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
	...
	//	integral_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
	if(type==0)
		trap_J0_J2<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    checkCudaErrors(cudaMemcpy(out, outc, nf*nc*sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(u_fc));
    checkCudaErrors(cudaFree(rc));
    checkCudaErrors(cudaFree(fc));
...
int FJ1(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
	...
	//	integral_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    if(type==0)
	trap_J1<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    checkCudaErrors(cudaMemcpy(out, outc, nf*nc*sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(u_fc));
    checkCudaErrors(cudaFree(rc));
    checkCudaErrors(cudaFree(fc));
...
int FJ2(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
	...
	//	integral_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
	if(type==0)
		trap_J2<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    checkCudaErrors(cudaMemcpy(out, outc, nf*nc*sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(u_fc));
    checkCudaErrors(cudaFree(rc));
    checkCudaErrors(cudaFree(fc));
...
//       integral_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    if(type==0)
	trap_Y1<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    checkCudaErrors(cudaMemcpy(out, outc, nf*nc*sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(u_fc));
    checkCudaErrors(cudaFree(rc));
    checkCudaErrors(cudaFree(fc));
...
int FY2(float *u_f,float *r, float *f,float *out, float *c,int nc,int nr,int nf,int type)
{
	...
	//	integral_Y<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
	if(type==0)
		trap_Y2<<<grid,block>>>(u_fc,rc,outc,cc,fc,nr,nc,nf);
    checkCudaErrors(cudaMemcpy(out, outc, nf*nc*sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(u_fc));
    checkCudaErrors(cudaFree(rc));
    checkCudaErrors(cudaFree(fc));
